...
- Railway's watched path for the `@leadsy/web` service is currently `/apps/web/**`. If a commit with watched app changes is skipped because GitHub CI failed, and a later CI-fix commit changes only non-watched files, Railway may skip the CI-fix commit with "No changes to watched files" even though production is stale.
- Before reporting production complete, compare the exact `origin/main` commit hash with the latest successful active Railway deployment commit hash. GitHub Actions passing is necessary but not sufficient.
- If production is stale after CI passes because Railway skipped a non-watched recovery commit, make an intentional tiny follow-up commit under `apps/web/**`, or use an explicit Railway redeploy only when the user asks for that emergency/manual path.
